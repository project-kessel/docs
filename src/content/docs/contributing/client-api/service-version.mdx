...
package:
    ...
    - name: ClientBuilder
      ...
      methods:
        ...
        - name: build
          ...
          returns:
            name: "{Service}Stub (and possibly the channel – see description)"
        - name: buildAsync
  ...
  functions:
    - name: get_root_workspaces
      description: |
        Gets the root workspaces for the specified organization.
        This function queries RBAC v2 to find the root workspaces for the given org_id.
        
        Example:
        
        ```python
        # Get all root workspaces
        root_workspaces = get_root_workspaces(
            rbac_endpoint="https://console.stage.redhat.com/", 
            psk="your-pre-shared-key",
            client_id="client-id",
            org_id="12345", 
            limit=20
        )
        for workspace in root_workspaces["data"]:
            print(f"Root workspace: {workspace['name']}")
            
        # Get just the first root workspace ID
        first_root = get_root_workspaces(
            rbac_endpoint="https://console.stage.redhat.com/",
            psk="your-pre-shared-key", 
            client_id="client-id",
            org_id="12345", 
        ...
        workspace_id = first_root["data"][0]["id"]
        ```
      params:
        - name: rbac_endpoint
          type:
        ...
        - name: psk
          type:
            name: string
          description: Pre-shared key for RBAC authentication. Will be sent as x-rh-rbac-psk header
        - name: client_id
          type:
            name: string
          description: Client identifier for the calling service. Will be sent as x-rh-rbac-client-id header
        - name: org_id
          type:
            name: string
        ...
        - name: limit
          type:
            name: integer
          optional: true
          description: "Maximum number of workspaces to return (default: 10)"
        - name: offset
          type:
            name: integer
          optional: true
      ...
      returns:
        name: "Dict[str, Any]"
        description: |
          A dictionary containing the workspace list response with structure:
          - `meta`: Pagination metadata (count, limit, offset, type)
          - `links`: Navigation links (first, next, previous, last) 
          - `data`: List of workspace objects with id, name, type, description, etc.

    - name: get_default_workspaces
      description: |
        Gets the default workspaces for the specified organization.
        This function queries RBAC v2 to find the default workspaces for the given org_id.
        
        Example:
        
        ```python
        # Get all default workspaces
        default_workspaces = get_default_workspaces(
            rbac_endpoint="https://console.stage.redhat.com/",
            psk="your-pre-shared-key",
            client_id="client-id",
            org_id="12345", 
        ...
        for workspace in default_workspaces["data"]:
            print(f"Default workspace: {workspace['name']}")
            
        # Get just the first default workspace ID
        first_default = get_default_workspaces(
            rbac_endpoint="https://console.stage.redhat.com/",
            psk="your-pre-shared-key",
            client_id="client-id", 
            org_id="12345", 
        ...
        workspace_id = first_default["data"][0]["id"]
        ```
      params:
        - name: rbac_endpoint
          type:
