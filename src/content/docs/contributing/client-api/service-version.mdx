      ...
      methods:
        ...
        - name: build
          description: |
            ...
            **Return Value**:
            - If the client has a `close()` method: return the client directly
            - If the client lacks a `close()` method: return a tuple/array `[Client, Closeable]` where `Closeable` is the resource that needs cleanup
