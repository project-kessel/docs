...
package:
  ...
  functions:
    - name: oauth2_auth
      description: |
        Create a requests-compatible authentication object that leverages the OAuth2 Client Credentials flow.
        This function provides the same functionality as the gRPC `oauth2CallCredentials` but for HTTP requests using a requests-compatible library.
      ...
      params:
        - name: credentials
          type:
            ...
            link: ../auth/#class-OAuth2ClientCredentials
          description: |
            OAuth2 client credentials instance that will be used for automatic token 
            retrieval and refresh for each HTTP request.
      returns:
        name: OAuth2Auth
        description: |
          A new instance of OAuth2Auth (subclass of requests.auth.AuthBase or similar library class) that automatically 
          handles OAuth2 token retrieval and injects Bearer tokens into HTTP requests.
    - name: fetch_root_workspace
      description: |
      ...
      params:
        ...
        - name: requests_module
          type:
            name: module
          description: |
            The requests module (or compatible library).
        - name: auth
          type:
            name: requests.auth.AuthBase
          description: |
            Requests-compatible authentication object. Use `oauth2_auth(credentials)` from 
            the `kessel.requests` package for OAuth2 authentication.
