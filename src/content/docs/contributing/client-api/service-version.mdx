...
package:
  classes:
    ...
    - name: UserAllowed
      description: |
        Enum representing the possible states of user authorization.
        Used to indicate whether a user is explicitly allowed, denied, or unspecified for a particular operation.

        Example:

        ```python
        if user_permission == UserAllowed.TRUE:
            # User is explicitly allowed
            process_request()
        elif user_permission == UserAllowed.FALSE:
            # User is explicitly denied
            deny_request()
        else: 
            # UserAllowed.UNSPECIFIED
            apply_some_default_policy()
        ```
      values:
        - name: UNSPECIFIED
          value: 0
          description: The authorization state is not specified or unknown
        - name: TRUE
          value: 1
          description: The user is explicitly allowed to perform the operation
        - name: FALSE
          value: 2
          description: The user is explicitly denied from performing the operation

  functions:
    - name: workspace_representation
      description: |
        ...
        ```python
        workspace_type = workspace_representation()
        ```
      params: []
      returns:
    ...
    - name: principal_resource
      description: |
        Function to create a ResourceReference for a user principal with domain prefix.
        Returns a protobuf ResourceReference with the user ID prefixed with "redhat/".

        Example:

        ```python
        user_principal = principal_resource("user123")
        ```
      params:
        - name: resource_id
          type:
            name: string
          description: The user identifier (will be prefixed with "redhat/")
      returns:
        name: ResourceReference
        description: A ResourceReference for the user principal
    ...
    - name: principal_resource_for_user_id
      description: |
        Creates a ResourceReference for a user principal based on user ID and domain.
        This function standardizes the creation of principal references for RBAC operations.

        Example:

        ```python
        user_resource = principal_resource_for_user_id("john.doe", "example.com")
        ```
      params:
        - name: id
          type:
    ...
    - name: principal_subject_for_user_id
      description: |
        ...
        This is a convenience function that wraps principal_resource_for_user_id to create a subject reference.
