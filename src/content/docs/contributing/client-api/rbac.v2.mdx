  ...
  functions:
    ...
    - name: listWorkspaces
      description: |
        ...
        # With a consistency token for read-after-write consistency
        consistency = Consistency(at_least_as_fresh=ConsistencyToken(token=my_token))
        for resp in listWorkspaces(inventoryClient, userSubject, "member", consistency=consistency):
            print(resp)
        ```
      params:
        - name: inventory
        ...
        - name: consistency
          type:
            name: Consistency
            link: https://buf.build/project-kessel/inventory-api/docs/main:kessel.inventory.v1beta2#kessel.inventory.v1beta2.Consistency
          description: |
            Optional consistency requirement for the request. The consistency requirement is
            applied to every page of a paginated request. See [Consistency model](/docs/building-with-kessel/concepts/consistency/) for more details.
          optional: true
      returns:
    ...
    - name: listWorkspacesAsync
      description: |
        ...
        # With a consistency token for read-after-write consistency
        consistency = Consistency(at_least_as_fresh=ConsistencyToken(token=my_token))
        async for resp in listWorkspacesAsync(inventoryClient, userSubject, "member", consistency=consistency):
            print(resp)
        ```
      params:
        - name: inventory
