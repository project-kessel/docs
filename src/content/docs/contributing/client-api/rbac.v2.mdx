  ...
  functions:
    ...
    - name: listWorkspaces
      description: |
        ...
        for workspace in listWorkspaces(inventoryClient, userSubject, "member"):
            print(f"User has access to workspace: {workspace.resourceId}")
        ```
      params:
        - name: inventory
          type:
      ...
      returns:
        name: "Iterable[ResourceReference]"
        link: https://buf.build/project-kessel/inventory-api/docs/main:kessel.inventory.v1beta2#kessel.inventory.v1beta2.ResourceReference
        description: An iterator of workspace resources that the subject has the specified relation to

    - name: listWorkspacesAsync
      description: |
        Lists all workspaces that a subject has a specific relation to.
        This function queries the inventory service to find workspaces based on the subject's permissions.
        This variant supports asynchronous iteration over results.

        Example:

        ```python
        userSubject = principalSubject("john.doe", "redhat.com")
        async for workspace in listWorkspacesAsync(inventoryClient, userSubject, "member"):
            print(f"User has access to workspace: {workspace.resourceId}")
        ```
      params:
        - name: inventory
        ...
        - name: relation
          ...
          description: The relationship type to check
      returns:
        name: "AsyncIterator[ResourceReference]"
        link: https://buf.build/project-kessel/inventory-api/docs/main:kessel.inventory.v1beta2#kessel.inventory.v1beta2.ResourceReference
        description: An async iterator of workspace resources that the subject has the specified relation to
---
