...
Client libraries depend on gRPC. The implementation varies by language:
- **Python, Ruby**: Use native C libraries (require pre-built binaries or compilation)
- **Go, Java, Node.js**: Pure language implementations (no native dependencies, always fast installation)

**Official Kessel SDKs:**

| Language | Repository | Pre-built Binaries |
|----------|-----------|-------------------|
| Python | [kessel-sdk-py](https://github.com/project-kessel/kessel-sdk-py) | ✅ Linux, macOS, Windows |
| Ruby | [kessel-sdk-ruby](https://github.com/project-kessel/kessel-sdk-ruby) | ✅ Linux, macOS, Windows |
| Node.js | [kessel-sdk-node](https://github.com/project-kessel/kessel-sdk-node) | N/A (pure JavaScript) |
| Go | [kessel-sdk-go](https://github.com/project-kessel/kessel-sdk-go) | N/A (pure Go) |
| Java | [kessel-sdk-java](https://github.com/project-kessel/kessel-sdk-java) | N/A (JVM-based) |

*Version requirements are maintained in each SDK repository.*

For languages using native libraries (Python, Ruby), pre-built binaries are available for all major platforms. Installation times are not affected by the native dependencies. Compilation from source only occurs on unusual platforms or when explicitly requested.

Node.js uses [@grpc/grpc-js](https://www.npmjs.com/package/@grpc/grpc-js), a pure JavaScript implementation with no native compilation needed.

<Aside type="tip">
For detailed guidance on SDK installation, compilation considerations, CI/CD optimization, Docker builds, and hermetic build setups, see [Setting up SDKs for CI](/docs/building-with-kessel/how-to/setup-sdk-for-ci/).
</Aside>
...
The _suffixes_ of these imports SHOULD be consistent from language to language (aside from language-specific delimiters). These packages are documented in detail following the links to the [per-package Client SDK specification](../client-api/guide):
