...
1. **First we have to tell Kessel about the resource type.**
   ...
   **In Kessel, a single resource can have multiple <i>representations</i> shared by different <i>reporters</i>.**
   ...
   The metadata for resources and reporters follows a directory structure.
    ...
    <Aside type="note">
    The schema language is actively evolving. Future versions will unify attributes and permissions into a single language with fewer files. The current approach shown here will continue to work.
    </Aside>
