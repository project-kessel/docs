...
These scripts are temporary until the RBAC API is completed.
</Aside>

The scripts below encapsulate the lower-level relationship tuples that Kessel uses internally.
Edit the configuration variables at the top of each script, then run them to set up your RBAC permissions.
