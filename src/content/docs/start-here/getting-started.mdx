...
2. **Compile to [SpiceDB](https://authzed.com/docs/spicedb/getting-started/discovering-spicedb) schema**.
    ...
    We install the `ksl` schema compiler earlier. Use it now to compile the schemas:
...
1. **Create a role**
    ...
    <Code code={createRoleScript} lang="bash" />
...
2. **Create a role binding**
    ...
    <Code code={createRoleBindingScript} lang="bash" />
...
This inherents the access granted at at the workspace level, as defined by the document schema.
