...
particularly the ones referring to permissions (ie: the view, edit, and delete relations from the example above that refer to workspace permissions.) The reporter and resource types likewise come from your schema- the reporter
type is defined by the folder the inventory schema file is placed in (see example above) and the namespace statement in the ksl file (see example above), which must match. The resource type is defined by the filename of
the inventory schema file and the type statement in the ksl file, which must match.
