...
Kessel does not distinguish between "data resources" (like documents) and "identity resources" (like users). They are all resources with types, identifiers, attributes, and relationships to other resources.

### Resource Types

A **resource type** defines the canonical type of a resource.
...
- Each reporter's representation is uniquely identified by the combination of `local_resource_id`, `resource_type`, `reporter_type`, and `reporter_instance_id` — individual IDs can be reused (e.g., multiple reporters can use the same `local_resource_id` for their different representations)
...
<Aside type="note">
  The correlation mechanism described below is not yet implemented. Currently, each reporter's representation creates a separate resource in Kessel.
</Aside>
