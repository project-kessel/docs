...
When a tenant is created, Kessel automatically creates 6 default role bindings to bootstrap access control — one for each combination of the two default groups and the three scopes. Each binding uses the platform role for that pair, which aggregates the seeded roles belonging at that scope:
