...
| **UNGROUPED_HOSTS** | "Ungrouped Hosts" | System workspace for hosts not assigned to any user workspace | Must have a parent |
...
```
Root Workspace (org: acme)
└── Default Workspace
    ├── Engineering (STANDARD)
    │   ├── Frontend Team (STANDARD)
    │   └── Backend Team (STANDARD)
    ├── Sales (STANDARD)
    └── Ungrouped Hosts
```
...
Permissions granted on a parent workspace automatically apply to all descendant workspaces. This is enforced by SpiceDB using the `t_parent` relation.
...
**SpiceDB permission evaluation:**

When checking if a user can perform action X on workspace W, SpiceDB:
1. Checks if the user has a role binding directly on W that grants permission X
2. If not, follows the `t_parent` relation to W's parent and checks again
3. Repeats up the tree until it finds a matching role binding or reaches the ROOT
...
- API endpoints require the `x-rh-rbac-org-id` header to identify the tenant

The ROOT, DEFAULT, and UNGROUPED_HOSTS workspaces are unique per tenant, enforced by a database constraint on `(tenant_id, type)`.
...
- `principal_resource_id` — The SpiceDB-compatible identifier: `{domain}/{user_id}` (e.g., `redhat/12345`)
...
For each principal in a group, SpiceDB stores a relationship tuple:
```
rbac/group:<group_uuid>#member@rbac/principal:<domain>/<user_id>
```

When a role binding targets a group, users get permissions by being members of that group.
...
When you report a resource, you can specify which workspace it belongs to. The inventory-api creates a SpiceDB relationship:
```
{namespace}/{resource_type}:{resource_id}#workspace@rbac/workspace:{workspace_id}
```

For example, a host might be assigned to the "Engineering" workspace:
```
hbi/host:host-123#workspace@rbac/workspace:eng-workspace-uuid
```

**How permissions work with resource-workspace relationships:**

When checking if a user can view `host-123`, SpiceDB:
1. Finds the workspace the host belongs to (via the `#workspace` relation)
...
All requests require the `x-rh-rbac-org-id` header to identify the tenant.
...
The inventory-api calls `SetWorkspace()` to create the SpiceDB relationship linking the resource to the workspace.
...
<LinkCard
  ...
  description="See examples of permission checks and resource reporting with workspaces."
