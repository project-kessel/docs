...
When replicating data, you need a reliable way to process the stream of data changes. A Kafka consumer is one way to solve this
problem. A consumer reads messages from Kafka topics by subscribing to topic partitions. The consuming application then processes
the messages to apply those changes.
...
### Considerations
For your own consideration in building a consumer there were a few guarantees that we wanted to hold true and may be relevant.
...
1. **Messages must be processed in order:**
  ...
  that causes unexpected consequences for the management fabric and service providers.
...
sasl-username: my-genric-consumer
...
```
This function synchronously retries a failing operation with an exponential backoff upto a configured maximum. 
...
``` 
The critical logic is within the `kafka.RevokedPartitions` case. When the consumer is notified
that it is about to lose ownership of its partitions, it calls `commitStoredOffsets()`. This
is essential to guarantee that all successfully processed messages before this point have been marked as complete.
This prevents the next consumer that receives the partition from reprocessing messages, ensuring at-least-once
