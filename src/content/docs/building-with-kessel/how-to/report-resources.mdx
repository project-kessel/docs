...
When using the outbox pattern with Kafka, a consumer is required to process events published by Debezium. 
A consumer reads messages from Kafka topics by subscribing to topic partitions. The consuming application then processes the message
...
and other related settings to provide the necessary credentials for a sucessful connection to Kafka.
...
is essential to guarantee that all sucessfully processed messages before this point have been marked as complete.
This prevents the next consumer that receives the parition from reprocessing messages, ensuring at-least-once
processing is maintaned.
