...
<Aside type="note" title="Role definitions do not change">
Note that the way these new permissions are associated with a role remains the same, i.e. via [role files](https://github.com/RedHatInsights/rbac-config/tree/master/configs/prod/roles).
</Aside>
...
Note that the principal id will differ based on the type of the principal:

* user principal \- `identity.User.Username`
* service account \- `identity.ServiceAccount.Username`
</Aside>
...
* see [Ephemeral Deployments with Kessel Inventory API](https://cuddly-tribble-gq7r66v.pages.github.io/using-kessel/inventory-api/ephemeral/) for information on how to deploy Kessel in the ephemeral environment.
...
Follow [Using Kessel in production](https://github.com/project-kessel/docs-internal/blob/a7e66c32d491880958c399584725267efbb678f6/src/content/docs/using-kessel/in-production.mdx) to set up your service to use Kessel in stage and prod.
