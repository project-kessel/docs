...
| `ALREADY_EXISTS` | Duplicate resource | Duplicate reporter key (same `resource_type` + `resource_id` + `reporter.type`) | Use update/report semantics or treat as success |
...
| `invalid reporter representation: …` | `INVALID_ARGUMENT` | Fix reporter struct in request |
| `invalid common representation: …` | `INVALID_ARGUMENT` | Fix common struct in request |
| `invalid resource at index …` / `invalid subject at index …` / `invalid relation at index …` | `INVALID_ARGUMENT` | Fix indexed item |
| `LookupObjects requires reporter type: …` | `INVALID_ARGUMENT` | Include reporter type in lookup request |
| `LookupSubjects requires subject reporter type: …` | `INVALID_ARGUMENT` | Include subject reporter type in lookup request |
